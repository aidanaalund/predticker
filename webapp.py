...
with header:
    ...
    percentage = (float(data['Close'].iloc[-1] - data['Close'].iloc[-2])/abs(data['Close'].iloc[-2]))*100.00
    if change > 0:
        st.header('${:0.2f}'.format(price)+':chart_with_upwards_trend:')
        string = '+${:0.2f}'.format(change)+'  (+{:0.2f}'.format(percentage)+'%)'
        st.caption(f':green[{string}]')
    elif change == 0:
        st.header('${:0.2f}'.format(price)+':arrow_right:')
        st.caption('+${:0.2f}'.format(change)+'  (+{:0.2f}'.format(percentage)+'%)')
    else:
        st.header('${:0.2f}'.format(price)+':chart_with_downwards_trend:')
        string = '-${:0.2f}'.format(abs(change))+'  (-{:0.2f}'.format(abs(percentage))+'%)'
